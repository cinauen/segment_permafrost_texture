...
PROC_STEP:
    ...
    2: - CALCULATE TEXTURE for whole imagery and statistics for full area and sub areas
    3: - SPLIT IMAGERY into tiles for CNN training and prediction
       - EXTRACT input for supervised classification and for test patches
         (no tiling required, extracts label and data full-area image using the specified AOIs)
    4: - CALCULATE TEXTURE on augmented training and non augmented prediction tiles
         (required for offline augmentation and prediction when using GLCM features)


!!! Note on data usage in this example:
    This example uses a limited amount of input data in order
    to reduce computation times and data size. Only one traning area A02
    and four test patches are used.
...
def get_proj_param(PROC_STEP, PATH_BASE, SCALE_TYPE):
    ...
    SCALE_TYPE: str (provided as function input, used for proc steps 2, 3, 4)
       Image input intensity scaling type for further processing (texture
       calc and segmentation).
    ...
    # ===== test patch  limit !!!!
    # to keep the example small we used here only a limited number of test patches
    # also only the trainng area A02 is used in this example !!!
    patch_limit = 4
    ...
    PARAM['FOLDER_NAME'] = 'BLyaE_v1'
    PARAM['SITE_NAME'] = 'BLyaE'   # site name
    # for output file prefixes
    PARAM['PROJ_PREFIX'] = PARAM['SITE_NAME'] + '_HEX1979'
    PARAM['EPSG_TARGET'] = 32654  # target coord syst

    # === AOI areas ===
    # AOI for clipping outer processing extent
    PARAM['AOI_full_area'] = 'BLyaE_full_area_AOI_buffered_32654_reduced.geojson'

    AOI_prefix = 'BLyaE_labelling_AOI_32654'
    PARAM['AOI_area_dict'] = {}
    # -- training areas --
    training_areas = [
        # 'A01',
    ...
    PARAM['AOI_area_dict'].update(
        {x: f'{AOI_prefix}_{x}.geojson' for x in training_areas})
    # -- test areas --
    ...
    # --- AOI sub areas ---
    # The sub areas further split the trainng areas
    # and are later used for cross-validation
    # (e.g. with training: [train-01, train-02], validation: ['train-03]
    #  and use each training sub-area once as validation)
    # Note: futher training areas can be added as additional dictionary items
    PARAM['SUB_AREAS_dict'] = {
        # 'A01': ['train-01', 'train-02', 'train-03'],
        'A02': ['train-01', 'train-02', 'train-03'],
        }
    PARAM['AOI_sub_area_dict'] = {
        f'{x}{yy}': f'{AOI_prefix}_{x}{yy}.geojson'
    ...
    if PROC_STEP in [1]:
        ...
        PARAM['GREYSCALE_FILE_INP'] = 'BLyaE_HEX1979_D3C1215-301069F003_georef_v1_reduced.tif'
        # output resolution
        ...
        PARAM['REF_IMG_PATH'] = None
        # clip area for hist match reference file (needs to be full path)
        # use None if no hist matching is done
        PARAM['AOI_REF_FILE_CLIP'] = None


    #  ---- TEXTURE CALCULATION -----
    if PROC_STEP in [2]:
        # Specify area over which to calculate texture
        # If want use the whole area then PARAM['AOI_TEX_SUFFIX'] should be named 'all'
        ...
        PARAM['STATS_FILE_SUFFIX'] = 'P02_tex_recalc_stats_file'
    ...
    if PROC_STEP in [3]:
        ...
        PARAM['LABEL_FILE_INP'] = {
            # 'A01': 'BLyaE_HEX1979_training_A01_labels_Lv01_class_certainty.tif',
            'A02': 'BLyaE_HEX1979_training_A02_labels_Lv01_class_certainty.tif',
            'test-01': 'BLyaE_HEX1979_test_patch1_labels_Lv01_class_certainty.tif',
            'test-02': 'BLyaE_HEX1979_test_patch2_labels_Lv01_class_certainty.tif',
            'test-03': 'BLyaE_HEX1979_test_patch3_labels_Lv01_class_certainty.tif',
            'test-04': 'BLyaE_HEX1979_test_patch4_labels_Lv01_class_certainty.tif',
            }
        ...
        #    {f'test-{x:02d}': 'BLyaE_HEX1979_test_labels_Lv01_class_certainty.tif'
    ...
    if PROC_STEP in [4]:
        ...
        # are initialized in PARAM04_augment_data
